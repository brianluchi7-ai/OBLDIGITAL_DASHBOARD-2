...
def obtener_datos():
    ...
    tablas = ["ftds_nov_2025", "ftds_oct_2025", "ftds_sep_2025"]
    dataframes = []
    ...
    if not dataframes:
        print("❌ No se generó FTD_MASTER (sin datos).")
        return pd.DataFrame()
    ...
    # Guardar CSV limpio
    df_master.to_csv("FTD_MASTER_preview.csv", index=False, encoding="utf-8-sig")
    print("💾 Vista previa guardada: FTD_MASTER_preview.csv")
    ...
    try:
        ...
        if conexion:
            ...
            cursor.execute("DROP TABLE IF EXISTS FTD_MASTER_CLEAN;")
            cursor.execute("""
                CREATE TABLE FTD_MASTER_CLEAN (
                    date TEXT,
                    id TEXT,
                    team TEXT,
                    agent TEXT,
            ...
            for _, row in df_master.iterrows():
                ...
                cursor.execute(
                    "INSERT INTO FTD_MASTER_CLEAN (date, id, team, agent, country, affiliate, usd, month_name) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                    valores
                )

            conexion.commit()
            conexion.close()
            print("✅ FTD_MASTER_CLEAN creada y poblada correctamente en Railway.")
    except Exception as e:
        print(f"⚠️ Error al crear FTD_MASTER_CLEAN: {e}")
